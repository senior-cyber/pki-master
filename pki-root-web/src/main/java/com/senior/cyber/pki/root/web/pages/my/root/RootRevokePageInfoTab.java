...
public class RootRevokePageInfoTab extends ContentPanel {
    ...
    protected void revokeButtonClick() {
        ...
        Optional<Certificate> optionalRoot = certificateRepository.findById(this.uuid);
        Certificate root = optionalRoot.orElseThrow(() -> new WicketRuntimeException("certificate is not found"));

        root.setRevokedDate(this.date_value);
        root.setStatus(CertificateStatusEnum.Revoked);
        certificateRepository.save(root);

        List<Certificate> monitor = new ArrayList<>();

        List<Certificate> certificates = certificateRepository.findByIssuerCertificate(root);
        List<Certificate> queue = new ArrayList<>(certificates);
        while (!queue.isEmpty()) {
            Certificate item = queue.remove(0);
            if (item.getStatus() == CertificateStatusEnum.Good) {
                monitor.add(item);
                List<Certificate> more = certificateRepository.findByIssuerCertificate(item);
                queue.addAll(more);
            }
        }

        for (Certificate certificate : monitor) {
        ...
        certificateRepository.saveAll(monitor);
