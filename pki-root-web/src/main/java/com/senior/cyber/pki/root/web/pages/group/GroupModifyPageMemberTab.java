...
public class GroupModifyPageMemberTab extends ContentPanel {
    ...
    protected void onInitData() {
        ...
        this.user_browse_provider.applyWhere("Group", Sql.column(UserGroup_.groupId) + " = :group_id", Map.of("group_id", this.uuid));
        this.user_browse_provider.setSort("full_name", SortOrder.ASCENDING);
        this.user_browse_provider.applyCount(Sql.column(UserGroup_.id));

        this.user_browse_provider.applySelect(String.class, "uuid", Sql.column(UserGroup_.id));

        this.user_browse_column = new ArrayList<>();
        {
            String label = "Name";
            String key = "full_name";
