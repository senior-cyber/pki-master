...
import com.senior.cyber.pki.root.web.utility.MemoryResourceStream;
...
public class RootBrowsePage extends MasterPage {
    ...
    protected FilterForm root_browse_form;
    protected MySqlDataProvider root_browse_provider;
    protected List<IColumn<Tuple, ? extends Serializable>> root_browse_column;
    protected DataTable<Tuple, Serializable> root_browse_table;
    ...
    protected void onInitData() {
        ...
        this.root_browse_provider.applyWhere("user", Sql.column(Certificate_.user) + " = :user", Map.of("user", session.getUserId()));
        this.root_browse_provider.applyWhere("type", Sql.column(Certificate_.type) + " IN (:type)", Map.of("type", types));
        this.root_browse_provider.applyCount(Sql.column(Certificate_.id));
        this.root_browse_provider.applySelect(String.class, "uuid", Sql.column(Certificate_.id));
        this.root_browse_provider.applySelect(Long.class, "serial", Sql.column(Certificate_.serial));
        ...
        this.root_browse_column = new ArrayList<>();
        {
            ...
            this.root_browse_column.add(this.root_browse_provider.filteredColumn(Date.class, Model.of(label), key, sql, serializer, filter, deserializer));
        }
        {
            String label = "Name";
            ...
            this.root_browse_column.add(this.root_browse_provider.filteredColumn(String.class, Model.of(label), key, sql, serializer, filter, deserializer));
        }
        {
    ...
    protected void download(Tuple tuple, Link<Void> link) {
        ...
        try {
            ...
            Optional<Certificate> optionalRoot = certificateRepository.findBySerial(serial);
            Certificate root = optionalRoot.orElseThrow(() -> new WicketRuntimeException("never happen"));
