...
public class SubordinateController {
     ...
     * @throws BadResponseException
     */
    @RequestMapping(path = "/subordinate/register", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<SubordinateRegisterResponse> subordinateRegister(RequestEntity<SubordinateRegisterRequest> httpRequest) throws IOException, CertificateException, NoSuchAlgorithmException, OperatorCreationException, ApduException, ApplicationNotAvailableException, BadResponseException, SignatureException, InvalidKeyException {
        SubordinateRegisterRequest request = httpRequest.getBody();
        if (request == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
        }
        ...
        SubordinateRegisterResponse response = this.subordinateService.subordinateRegister(request, this.crlApi + "/" + serial + ".crl", this.ocspApi + "/" + serial, this.x509Api + "/" + serial + ".der");
        return ResponseEntity.ok(response);
    }
