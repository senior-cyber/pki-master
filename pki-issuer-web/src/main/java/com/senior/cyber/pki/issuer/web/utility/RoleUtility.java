...
public class RoleUtility {
    ...
    public static List<String> lookupRole(NamedParameterJdbcTemplate named, String userId) {
        ...
        String userQuery = "SELECT " + Sql.column(Role_.name) + " FROM " + Sql.table(Role_.class) + " INNER JOIN " + Sql.table(UserRole_.class) + " ON " + Sql.column(UserRole_.roleId) + " = " + Sql.column(Role_.id) + " WHERE " + Sql.column(UserRole_.userId) + " = '" + userId + "' AND " + Sql.column(Role_.enabled) + " = true AND " + Sql.column(Role_.id) + " NOT IN (" + deniedQuery + ")";
